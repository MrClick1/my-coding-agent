...
一个用于学习编程 Agent 底层机制的轻量项目。当前已经完成只读文件浏览、代码搜索和单任务状态约束，后续将逐步加入精确修改和测试验证，而不是一开始堆叠复杂框架。
...
当前仍故意不注册文件写入和任意 Shell 工具，因此 Agent 只能检查、搜索和解释代码，不能修改项目。“先读后写”规则已经进入工具注册表，下一阶段加入 `replace_text` 时会直接受到该规则保护。
...
`search_code` 默认不区分大小写，可以限定单个文件或目录；默认最多返回 50 条结果，可在 1 到 100 之间设置。三个工具都会遵守工作区边界与敏感文件过滤，并分别设置结果数、扫描量或输出大小上限。
