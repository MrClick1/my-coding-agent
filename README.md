...
一个用于学习编程 Agent 底层机制的轻量项目。当前已经完成持续聊天、文件浏览、代码搜索、单任务状态约束、精确文本修改、修改预览与人工确认、修改后测试验证和可复现基准测试，而不是一开始堆叠复杂框架。

代码参考 `agent-tutorial` 等项目的学习思路，但采用独立结构重新实现。

## 当前阶段：持续聊天式 Coding Agent
...
- `/help`：显示会话命令；
- `/clear`：清除对话历史和最近一轮工具状态；
- `/exit`：退出会话。

会话默认保留最近 20 轮“用户目标 + 最终回答”，并限制为 8 万字符。旧工具输出不会永久加入历史，避免上下文无限增长；如果后续任务依赖文件内容，Agent 仍需重新使用工具检查。每一轮都会重置已读文件和当轮测试结果，上一轮读过文件不能绕过下一轮的先读后写规则。如果上一轮已经写入但在测试前异常结束，待验证修改会跨轮保留，下一轮仍不能绕过测试门禁。历史目前只保存在当前进程内存中，退出会话后不会持久化。
...
9. 下一步可增加任务级修改日志与回滚，让已批准的多次修改也能统一审计和撤销。
