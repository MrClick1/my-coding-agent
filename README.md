...
一个用于学习编程 Agent 底层机制的轻量项目。当前已经完成文件浏览、代码搜索、单任务状态约束、精确文本修改、修改预览与人工确认、修改后测试验证和可复现基准测试，而不是一开始堆叠复杂框架。
...
当前只提供 `replace_text` 这一种受控写入方式：它不能创建或删除文件，也不会在缺少旧内容校验时直接覆盖整个文件。旧文本的实际出现次数必须与预期完全一致，修改后的文件不能超过 1 MB，完整预览不能超过 3 万字符。用户批准后还会重新读取文件；如果文件在确认期间发生变化，写入会被取消。最终修改通过同目录临时文件进行原子替换。项目仍不提供任意 Shell。
...
这条命令会产生真实模型 API 调用，可能产生费用，所以普通离线测试不会自动执行它。基准 fixture 是用完即删除的隔离副本，因此基准运行器会显式自动批准其中的替换，不会弹出人工确认。运行结果会显示逐案例成败和整体成功率，完整 JSON 默认写入 `benchmark-results/latest.json`；该目录已被 Git 忽略。案例格式位于 `benchmarks/cases.json`，可以通过 `--manifest` 和 `--output` 指定其他清单与报告路径。
...
8. 下一步可增加任务级修改日志与回滚，让已批准的多次修改也能统一审计和撤销。
