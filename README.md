...
一个用于学习编程 Agent 底层机制的轻量项目。当前已经完成文件浏览、代码搜索、单任务状态约束和精确文本修改，后续将继续加入测试验证，而不是一开始堆叠复杂框架。
...
自然语言任务
    -> OpenAI 兼容模型
    -> 工具调用（Tool Calling）
    -> list_files / search_code / read_file / replace_text
    -> AgentState 记录本轮读取与修改状态
    -> 工具结果回传模型
    -> 最终回答
```
...
`search_code` 默认不区分大小写，可以限定单个文件或目录；默认最多返回 50 条结果，可在 1 到 100 之间设置。所有工具都会遵守工作区边界与敏感文件过滤，并分别设置结果数、扫描量、读取量或输出大小上限。
...
├── state.py          # 单任务状态与先读后写约束
├── tooling.py        # 工具定义、注册和执行
└── workspace.py      # 工作区安全与目录、搜索、读取、精确替换工具
tests/                # 不需要真实模型的离线测试
```
...
Agent 必须先调用 `read_file`，随后才能调用 `replace_text`。如果旧文本出现次数与预期不一致，工具会拒绝修改并要求模型重新检查文件。
